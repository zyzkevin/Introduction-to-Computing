...
bool Move(int x0, int y0, int x1, int y1, int x2, int y2, int color, bool is_blocked)  //张俊优 1800094804
{
    if ((!inMap(x0, y0)) || (!inMap(x1, y1)) || (!inMap(x2, y2)))
        return false;
    else if (gridInfo[x0][y0] != color || gridInfo[x1][y1] != 0)
        return false;
    else if ((gridInfo[x2][y2] != 0) && !(x2 == x0 && y2 == y0))
        return false;
    else if (!is_blocked)
    {
        gridInfo[x0][y0] = 0;
